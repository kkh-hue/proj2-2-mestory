...
| EC-11 | 질문의 설비 종류가 정식 이름이 아니라 흔한 줄임말("사출기")이다 | `_TYPE_ALIASES`(`backend/scope.py`)에 있는 표현이면 정식 이름으로 인식해 EC-08과 같게 처리. 사전에 없는 표현은 추측하지 않고 종류 이름을 무시(EC-08의 "0대"와 동일) |

**EC-11에 대한 참고**: 이건 검색 결과를 LLM 프롬프트에 넣는 RAG가 아니다. `resolve_scope()`는 순수 함수로
"어느 설비를 조회할지"만 정하고, 그 결과(설비 ID)만 다음 단계(MCP 조회)로 넘어간다. 임베딩 유사도도 아니고,
사람이 미리 적어 둔 사전(alias)만 본다 — 모르는 표현을 추측해서 엉뚱한 설비를 조회하면 안 되기 때문이다.
