...
- 담당: 박민영 · 상태: **구현 완료** (PR #87·#88 머지)
  - `9490eb4` 채팅 입력창에 이미지 첨부 / `1f19b0e` 답변에 이미지에서 읽어낸 내용 표시
  - `99d8bb1` 대화 전환 시 보내지 않은 첨부 초기화
...
- ✅ `frontend/`는 강경희 님 영역 → 측정 근거를 밝히고 리뷰받아 머지 완료.
...
- **왜 지금 하나 (측정 근거)**: `evals/runs/before.json` 실측에서 이미지를 넣었을 때 `visual_extraction 0.900`, 같은 평가셋에서 이미지만 뺀 대조군은 `0.500`이었다 — **차이 +0.400**. 지연은 `14.2초`로 당시 이미지 게이트(20초) 안이었다. 기능이 실제로 일하고 있고 게이트도 통과하는데, 화면이 없어서 못 쓰고 있다.

  > ⚠️ **이 문단의 숫자는 착수 시점(gpt-4o-mini)의 근거다.** ZDR 때문에 `openai/gpt-5-mini`로
  > 바꾼 뒤 지연이 약 2배가 됐고, 게이트도 **최악1건 28초**로 다시 잡았다. 이미지 기여도는
  > 여전히 유효하며 오히려 커졌다(**+0.400 → +0.400~+0.500**). 최신 수치는
  > **`evals/EVAL_REPORT.md`** 를 볼 것. 또 `p95`라고 부르던 값은 케이스 10건에서
  > 사실상 **가장 느린 1건**이라 이름을 `최악1건`으로 고쳤다.
- **측정 지표**:
...
- 구현은 PR #87·#88로 머지됐다. 위 AC는 **그 PR 시점에 확인한 것**이다.
- 이 문서를 갱신한 2026-09-21에는 **수동 AC(F01~F10)를 다시 확인하지 않았다.** 상태 표기만 고쳤다.
- 그 뒤 모델이 `gpt-5-mini`로 바뀌었으므로, 화면 동작을 다시 보증하려면 수동 AC를 재확인해야 한다.
  특히 AC-F02(읽어낸 내용 표시)·AC-F09(빈 `visual_findings`)는 모델 출력에 직접 의존한다.
- `AC-F13`(pytest)은 2026-09-21에 확인했다 — `145 passed, 1 failed, 4 skipped`.
  실패 1건은 `tests/test_alert_names.py`의 날짜 의존 문제로 프론트와 무관하다.
