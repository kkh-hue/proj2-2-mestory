...
| `user_id` · `session_id`를 붙였는가 | ⚠️ 부분적 — 코드 수정함, 배포 후 확인 필요. `session_id`는 모든 화면 경로(채팅·리포트 생성)와 평가(`eval-<회차>`)에 붙는다 — 비어 있는 211건은 주로 붙이기 전 트레이스다. **`user_id`는 평가 실행(`eval-runner`)에만 붙고 서비스 요청에는 붙지 않았다** — 로그인이 없고 `main.py`가 `generate_report()`에 `user_id`를 넘기지 않았다. `user_id`를 붙이는 코드가 평가 스크립트뿐이라 400건 중 `user_id` 145건은 전부 평가이고, `session_id`만 있는 44건(189−145)이 서비스 요청으로 추정된다. 서비스 요청에 `user_id="web"`(요청 출처)을 붙이도록 고쳤다(`docs/specs/langfuse-user-id.md`). 과거 트레이스는 누가 보낸 요청인지 기록이 없어 소급하지 않는다 — **배포 시점을 기준선으로 "그 이후 N건 중 N건"을 적을 것.** |
