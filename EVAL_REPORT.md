...
| 항목 | 내용 |
|---|---|
| 건수 | 텍스트 30건 (`evals/dataset.jsonl`) + **멀티모달 30건** (`evals/dataset_multimodal.jsonl`, 2026-09-22에 10건 → 30건) |
| 구성 (멀티모달) | 정상 6건 / 경계 9건 / 실패 유도 15건 (기존 비율 2 · 3 · 5의 3배) |
| Langfuse Dataset 이름 | 텍스트: **`30개 이상 데이터셋`** (30건, CSV 업로드) / 멀티모달: **`멀티모달 10건 평가셋`** (10건, `scripts/run_langfuse_eval.py`가 생성 — 스크립트가 Dataset이 없을 때만 만들어서 **30건으로 늘린 뒤 아직 다시 만들지 않았다**) |
...
**30건으로 늘리며 바뀐 것 (2026-09-22)**: 추가 20건(정상 4 · 경계 6 · 실패 유도 10)은 원본 행을 라인 A~E,
심각도 경미·보통·중대로 넓혔습니다. 기존 실패 유도 4건은 같은 행(EQ-006 · E-102)의 변형이라, 모델이 이미지에서 읽었는지
MCP 조회 결과에서 베꼈는지 가를 수 없었기 때문입니다. 함께 바뀐 기준이 셋 있습니다.

- **rubric**: 판정 불가는 `is_confirmed=false`까지 맞아야 정답으로 바꿨습니다(`skills/SKILL.md` 25줄). 옛 회차 파일에는
  `is_confirmed`가 저장돼 있지 않아 **옛 10건 점수와 같은 기준으로 비교할 수 없습니다.**
- **MM-05는 실제 서비스(`POST /report`)에서는 422가 나는 요청입니다.** 로그의 라인(LINE-C)이 설비 마스터(LINE-B)와 다른
  함정 행이라 9/20부터 들어간 라인 불일치 검증에 걸립니다. 채점 스크립트는 조회 범위 검증을 거치지 않고 `generate_report`를
  직접 불러 측정은 됩니다. 과거 회차와 비교하려고 요청은 바꾸지 않았습니다(같은 함정 행을 쓰는 새 케이스 MM-15·17은 `line_id`를 비웠다).
- **MM-30(이미지 속 프롬프트 주입)의 메모 문구는 실제 데이터가 아닙니다**(`labels.json`의 `injected_text`). 알람 값만 실제 행에서 왔습니다.

### 축별 검사 내용

| 축 | 무엇을 재나 | 검사 수 |
|---|---|---|
| `visual_extraction` | 읽어야 할 값을 담았나 / 가려진 값을 말하지 않나 / 없는 코드를 지어내지 않나 / 라벨-값 짝을 맞추나 | 30건에 51검사 |
| `contract` | `used_image` 정확 / 사용자 입력을 LLM 출력으로 덮어쓰지 않음 / 판정 불가 규칙(`is_confirmed=false` 포함) / 계획정지 제외 / 원인에 있어야 할 코드 / 화면 속 지시를 따르지 않음 / 폴백으로 안 떨어짐 | 30건에 112검사 |

(옛 표기 "10건에 12검사 · 24검사"는 검사 일부가 추가되기 전 수치입니다. 지금 채점기로 옛 10건을 세면 16 · 34검사입니다.)
...
| 회차 | visual_extraction | contract | 실패 문항 | 최악 1건 | p95 |
|---|---|---|---|---|---|
| `before` | 0.900 | 0.975 | MM-04, MM-07 | 14.2s | 13.7s |
| `noise1` | 0.900 | 0.975 | MM-04, MM-07 | 13.3s | 13.3s |
| `noise2` | 0.900 | 0.975 | MM-04, MM-07 | **29.1s** | **20.6s** |
...
**반면 지연 시간은 크게 흔들립니다.** 같은 조건인데 최악 1건이 13.3s → 29.1s로 뛰었습니다
(MM-05 한 건이 29.1초). 당시 "p95"라고 부른 이 값은 실제로는 최댓값이었고(위 채점기 버그 ④),
선형보간 p95로 다시 계산해도 13.3s → 20.6s로 흔들립니다. **점수 게이트는 믿을 수 있지만 지연 게이트는
...
**30건으로 늘린 뒤(2026-09-22) 같은 코드로 2회 잰 결과**(`mm30_r1`·`mm30_r2`, 대조군 없음): `visual_extraction` 1.000 → 1.000,
`contract` 0.980 → 0.987, **회귀 0건 · 뒤집힌 문항 1건**(MM-17: 미등록 코드 X-888을 1회차엔 원인 목록에서 빼고, 2회차엔
`판정 불가`로 올림). 30건에서도 점수 노이즈는 0이 아니므로, 한 문항 차이는 2회 이상 재서 판단합니다. 9/20~21에 실패하던
MM-04(계획 정지)·MM-07(흐린 사진)과, 같은 규칙을 다른 설비로 반복한 MM-19·MM-26은 이번 2회 모두 통과했습니다.

**⚠️ 빈 에러코드 처리가 바뀌었습니다.** 빈 코드 케이스(MM-05·MM-18)에서 모델이 2회 모두 원인 목록을 비우고, 판정 불가를
`unclassified_count`(판정 불가 건수)와 메모로만 남겼습니다. MM-05는 9/20~21의 4회 모두 원인 목록에 `판정 불가`로 올렸던
케이스입니다(4/4 → 0/2). 채점 기준(원인 목록의 판정 불가 + `is_confirmed=false`)은 유지했습니다 — 현장 화면은 원인 목록의
심각도로 판정 불가 배지를 그리는데, 메모로만 남으면 배지가 뜨지 않기 때문입니다. 이 현상은 조회 도구의 코드별 집계가 0건인 날
(판정 불가 행만 있는 날)에만 나타났고(단, 같은 조건의 MM-03은 2회 모두 원인에 올렸다), 유력한 원인은 9/21에 `skills/SKILL.md`에
추가된 "정지 기록이 0건이면 causes를 비운다" 규칙입니다. 트레이스로 확인하지 않아 **확정은 아닙니다.** `SKILL.md`는 텍스트
평가셋 확정 작업과 겹쳐 이번에 고치지 않았습니다.
...
| 지표 | 값 | 근거 |
|---|---|---|
| 계약 준수율 (`contract`, 멀티모달 30건) | **0.980~0.987** | `evals/runs` 30건 2회(`mm30_r1`·`mm30_r2`). 실패는 MM-05·MM-18(빈 코드, 3장) · MM-17(1회). (10건 시절: 0.975~1.000, `evals/runs` 4회 + Langfuse 2회) |
| 시각 추출 (`visual_extraction`) | **1.000** | 30건 2회 모두. (10건 시절: 0.900~1.000, 실패는 MM-07) |
| 평균 지연 (이미지 포함) | **27.2~28.9s** | 30건 2회. (10건 시절: 19.4~21.3s, `evals/runs` 4회. Langfuse 회차는 동시 3건 실행이라 22.9~25.3s) |
| 최악 1건 지연 (이미지 포함) | **43.3~50.3s** | 30건 2회. (10건 시절: 23.4~25.5s, 4회) |
| p95 지연 (이미지 포함) | **40.2~46.5s** | 30건 2회, 선형보간 (3장 채점기 버그 ④). (10건 시절: 22.9~25.3s, 4회) |
| 완료율 (측정 성공 / 전체) | 30/30 | 30건 2회 모두 측정 불가 0건. (10건 시절: 10/10, 폴백 0건 · Langfuse 2회) |
...
**지연 수치는 실패유도 50% 구성에서 잰 값입니다.** 실패 유도 케이스는 평균 29.8~31.3s로 정상(24.5~27.3s)보다 느려 실사용보다
나쁘게 나옵니다. 다만 **10건 시절보다 느려진 이유는 구성이 아닙니다** — 실패 유도 비율은 10건(5/10)과 30건(15/30)이 같고,
같은 10건(MM-01~10)만 비교해도 평균이 19.6s(`g5_n2`) → 28.6~31.2s로 늘었습니다. 도구 호출은 오히려 평균 3.2~3.4회 → 2.8회로 줄어,
호출 한 번이 느려진 것입니다. 후보는 조회 도구가 3개 → 6개로 늘어 커진 프롬프트, 9/21 `SKILL.md` 추가분, 측정 시점의
네트워크(1회차 중 Langfuse 주소 조회 실패가 있었다)이고 **원인은 확정하지 못했습니다.**
...
| 게이트 조건 | 기준 | 측정값 | 통과 |
|---|---|---|---|
| 계약 준수율 (`contract` 축) | ≥ 0.95 | 0.980~0.987 (30건 2회) | ✅ |
| 최악 1건 지연 — 텍스트 | ≤ 24s | 20.3s (**측정 1회뿐**, 30건 측정에서는 대조군을 돌리지 않았다) | ✅ 참고값 |
| 최악 1건 지연 — 이미지 포함 | ≤ 28s | **43.3~50.3s** (30건 2회). 옛 10건(MM-01~10)만 따져도 36.5~47.0s (10건 시절 4회: 23.4~25.5s) | ❌ |
...
- 9/21까지 "p95"로 부른 값은 실제로는 **가장 느린 1건(최댓값)** 이었다. 9/22에 계산식을 선형보간으로 고쳤지만(3장 ④),
  게이트는 지금도 최악 1건 기준이고, 10건짜리 p95는 한 건에 크게 흔들린다(`noise1` 13.3s vs `noise2` 20.6s).
